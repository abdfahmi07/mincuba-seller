    ...
    try {
      ...
      const formattedImages: PreviewImage[] = resultData.ProductImage.map(
        (image: ProductImage) => {
          ...
          const pathName = splittedUrl[splittedUrl.length - 1];
      ...
      reset(
        {
          ...
          condition:
            conditionOptions.find(
              (option) => option.value === resultData.condition
            ) ?? null,
          unit:
  ...
  const editProduct = async (data: EditProductFormValues) => {
    try {
      const imageFiles = data.images
        ...
        .map((image: PreviewImage) => image.file as File);

      const uploadedImageUrls = await uploadAllImages(imageFiles);

      const filterredImagesUrl = data.images
        .filter((image: PreviewImage) => !(image.file instanceof File))
        .map((image: PreviewImage) => image.url);

      const imagesUrls = [...filterredImagesUrl, ...uploadedImageUrls];
